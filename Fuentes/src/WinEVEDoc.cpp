...
//			AfxMessageBox(txt);
	...
	case (WORD32)9:
		...
		status=1;
		break;
	case (WORD32)10: // read key without blocking
		cpu->mm[8]  = cpu->lstChr; // inject last char pressed in terminal, no matter when it was pressed
		cpu->lstChr = 0;
		break;
	case (WORD32)20: // get random int
		if (fp.u == 0) {  // parameter == 0 => random seed
			srand (time(NULL));
		}
		else {
			*(WORD64 *)&cpu->mm[8] = WORD64(rand() % fp.u);
		}
